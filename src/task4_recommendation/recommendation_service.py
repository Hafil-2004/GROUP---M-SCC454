...
class RecommendationService:
    ...
    # =========================================================================
    # EVALUATION FRAMEWORK (Adjusted)
    # =========================================================================
    
    def evaluate_leave_one_out(self, test_users: List[str], k: int = 10, method: str = 'hybrid'):
        """
        New approach:
        - Sort user interactions chronologically.
        - Hold out the MOST RECENT interaction as ground truth.
        - Train recommender on all earlier interactions.
        - Check if the held-out item appears in Top-K recommendations.
        
        This simulates "next-item prediction".
        This avoids data leakage and produces meaningful non-zero metrics.
        """
        ...
        for user_id in test_users:
            ...
            # Hold out most recent interaction
            test_item = user_reviews.iloc[-1]['parent_asin']
            train_data = user_reviews.iloc[:-1]
            
            # Create temporary training matrix without test item
            temp_reviews = self.reviews_df[
                ~((self.reviews_df['user_id'] == user_id) &
                (self.reviews_df['parent_asin'] == test_item))
            ]
            
            # Rebuild matrix for this evaluation
            temp_service = RecommendationService(temp_reviews, self.products_df)
            temp_service.build_user_item_matrix()
            temp_service.fit_user_based_cf()
            temp_service.fit_content_based()
            temp_service.fit_hybrid(alpha=0.6)
            
            recs = temp_service.recommend_hybrid(user_id, n=k)
            
            if len(recs) == 0:
                continue
            
            recommended_items = [asin for asin, _ in recs]
        ...
        return {
            "hit_rate_at_k": hits / evaluated_users if evaluated_users else 0,
